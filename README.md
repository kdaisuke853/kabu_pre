...
## 制作背景
現在または1年間の株価を取得して、グラフ表示や株価予測ができるアプリケーションです。
...
┣ aws-ecs 本番環境用ファイル  
     &ensp;┠db_ecs DBイメージ  
     &ensp;┠django-fb Djangoファイル(時系列予測のみ、今後複数の機械学習のフレームワークを追加予定)  
     &ensp;┠django-fbuwsgi Djangoファイル本番使用(時系列予測とその他機能統合用)  
     &ensp;┠djnago-uwsgi Djangoファイル(予測機能以外)  
     &ensp;┗ nginx-ecs (Nginxファイル)  
┣ django2 ローカル用    
┣ nginx ローカル用Nginxファイル    
┗vue フロント部分(vue)  
